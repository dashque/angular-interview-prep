const tasks = [
  ...
  {
    id: 't11',
    title: 'Graceful degradation на экране',
    level: 'BFF · устойчивость',
    prompt:
      'Страница товара собирает данные из трёх сервисов: сам товар, отзывы и «похожие товары». Сервис отзывов иногда недоступен. Продакт хочет, чтобы страница всё равно открывалась. Как спроектировать это на стороне BFF и Angular, и какие термины назвать?',
    answer:
      'Ключевая идея — graceful degradation: частичный ответ вместо полного падения. На BFF ставим таймаут на вызов сервиса отзывов и circuit breaker, чтобы не копить каскад отказов; если отзывы не пришли, BFF отдаёт страницу с product и similar, а reviews делает null, а не роняет весь ответ 500-й. На стороне Angular модель экрана должна допускать отсутствие блока: reviews — это отдельный сигнал, который может быть null, и шаблон через @if показывает либо список отзывов, либо аккуратную заглушку «отзывы временно недоступны». Важно не смешивать «отзывов нет» и «сервис отзывов недоступен» — это разные состояния. На интервью стоит явно произнести: timeout, circuit breaker, graceful degradation, частичный ответ, и подчеркнуть, что критичные для экрана данные (сам товар) и некритичные (отзывы) обрабатываются по-разному.',
  },
  {
    id: 't12',
    title: 'Откуда брать user_id',
    level: 'BFF · безопасность',
    prompt:
      'На код-ревью вы видите, что фронтенд отправляет запрос GET /api/orders?userId=123, и бэкенд по этому userId отдаёт заказы. Что здесь не так с точки зрения безопасности и как правильно?',
    answer:
      'Проблема в том, что userId приходит из клиента и его можно подменить: поменяв параметр на чужой id, пользователь получит чужие заказы (это IDOR / нарушение авторизации). Идентификатор пользователя нельзя доверять из тела или query-параметров запроса — он должен извлекаться только из проверенного JWT на стороне BFF или gateway. Правильная схема: Angular кладёт токен в заголовок Authorization: Bearer … (обычно через HTTP interceptor), BFF валидирует токен, сам достаёт user_id из claims и подставляет его во внутренние вызовы. Клиентский код вообще не должен передавать свой user_id — сервер знает его из токена. Заодно стоит упомянуть, что interceptor — удобное место, чтобы добавлять и Authorization, и Correlation ID, не размазывая это по сервисам.',
  },
  {
    id: 't13',
    title: 'Куда положить новую фичу в модульной структуре',
    level: 'DDD Strategic',
    prompt:
      'Проект разбит на домены и поддомены, внутри — фичи со своими формами, facade и сервисами. Приходит задача: добавить новый экран-форму, который по смыслу принадлежит одному поддомену, но использует пару справочников из другого. Как решить, где его разместить и как подключить чужие данные, чтобы не размыть границы?',
    answer:
      'Ориентируйтесь на язык и ответственность: экран живёт в том поддомене, чьей задаче он служит и чьим языком описывается, — там же его facade, модель и сервисы. Чужие справочники — это зависимость на другой Bounded Context, и тянуть их модель «как есть» нельзя, иначе границы размоются. Правильный ход — обращаться к чужому контексту через узкий контракт (сервис или клиент того поддомена) и переводить его DTO в свою модель через маппер (ACL) прямо на входе в фичу. Тогда изменение чужого справочника бьётся об один маппер, а не расползается по форме. Если справочник по-настоящему общий и стабильный (единицы измерения, коды, Money), его можно оформить как маленький Shared Kernel, но только по явной договорённости команд. Для интервью формула такая: фича — в своём контексте; чужие данные — через контракт плюс ACL; общий стабильный минимум — Shared Kernel.',
  },
  {
    id: 't14',
    title: 'Facade или состояние прямо в компоненте',
    level: 'GRASP · SOLID',
    prompt:
      'В форме с несколькими вкладками и зависимыми полями логика (загрузка справочников, пересчёт зависимых значений, отправка) начала накапливаться прямо в компоненте, и он разросся. Коллега предлагает оставить как есть — «так меньше файлов». Как обосновать выделение facade и что именно туда вынести?',
    answer:
      'Разросшийся компонент нарушает сразу SRP (у него несколько причин меняться: вёрстка, сценарий, контракт API) и GRASP High Cohesion. Компонент должен остаться тонким Controller представления — принимать пользовательские события и показывать состояние, а сам сценарий экрана (загрузка справочников, оркестрация зависимых пересчётов, сохранение, обработка ошибок) вынести в facade. Тогда логику можно тестировать без DOM, вёрстку можно менять не трогая сценарий, а детали HTTP прячутся ещё ниже в сервисах. «Меньше файлов» — ложная экономия: цена всплывёт на первом же изменении контракта или бизнес-правила, когда придётся разбираться в огромном компоненте. Аргумент для интервью: компонент — тонкий Controller; facade — координатор сценария (High Cohesion, SRP); данные и HTTP — отдельный слой (Information Expert плюс Pure Fabrication).',
  },
  {
    id: 't15',
    title: 'Что тестировать в сервисе формы без бэкенда',
    level: 'SOLID · тестируемость',
    prompt:
      'Нужно покрыть тестами сервис формы, который грузит данные, пересчитывает зависимые поля и готовит payload на сохранение. Реального бэкенда в тесте нет. Что и как тестировать, и что в архитектуре должно это позволять?',
    answer:
      'Тестировать надо поведение сценария, а не реализацию HTTP: при таких-то входных данных сервис выставляет такое-то состояние; при ошибке загрузки переходит в состояние ошибки, а не падает; зависимые поля пересчитываются по правилам; payload на сохранение собирается корректно из текущего состояния формы. Чтобы это было возможно без бэкенда, сервис не должен сам создавать HttpClient и знать конкретные URL — зависимость от API приходит через DI за абстракцией (DIP), и в тесте вместо реального клиента подставляется заглушка (fake или mock), возвращающая нужный ответ или ошибку. Чистые функции пересчёта и маппинга стоит вынести отдельно и тестировать как обычные функции — им не должен быть нужен Angular вообще. Формула для интервью: зависимости — через DI за интерфейсом (DIP), сценарий — на замоканном клиенте, чистая логика (пересчёт, маппинг) — юнит-тестами без фреймворка.',
  },
];
...
'Отлично: агрегация под экран собрана'`,
  ...
  {
    id: 'c7',
    title: 'ACL: DTO со списком → view-model с нумерацией',
    blurb:
      'Частая задача: перевести ответ бэкенда со вложенным списком в модель для формы. Напишите buildRouteVm(dto): у dto есть id и необязательный массив points из объектов { code, name }. Верните { id, points }, где каждый пункт получает сквозной порядковый номер order, начиная с 1. Если points отсутствует — верните пустой массив points.',
    starter: `function buildRouteVm(dto) {
  // dto: { id: string, points?: Array<{ code: string, name: string }> }
  // вернуть: { id, points: Array<{ order: number, code: string, name: string }> }
}`,
    hiddenTests: `
const full = buildRouteVm({ id: 'R1', points: [{ code: 'AAA', name: 'A' }, { code: 'BBB', name: 'B' }] });
if (full.id !== 'R1') throw new Error('id должен переноситься из dto');
if (full.points.length !== 2) throw new Error('Ожидалось 2 пункта');
if (full.points[0].order !== 1 || full.points[1].order !== 2) throw new Error('order должен идти с 1 по порядку');
if (full.points[0].code !== 'AAA' || full.points[1].name !== 'B') throw new Error('code/name должны сохраняться');

const empty = buildRouteVm({ id: 'R2' });
if (empty.id !== 'R2') throw new Error('id должен переноситься даже без points');
if (!Array.isArray(empty.points) || empty.points.length !== 0) throw new Error('Без points ожидался пустой массив');
'Отлично: список замаплен и пронумерован'`,
  },
  {
    id: 'c8',
    title: 'Список: переместить пункт (иммутабельно)',
    blurb:
      'В форме со списком пунктов есть кнопки «вверх/вниз». Напишите moveItem(list, from, to): верните НОВЫЙ массив, в котором элемент с индекса from перемещён на позицию to. Исходный массив мутировать нельзя. Если from или to вне диапазона — верните неизменённую копию.',
    starter: `function moveItem(list, from, to) {
  // вернуть новый массив; исходный не мутировать
}`,
    hiddenTests: `
const src = ['a', 'b', 'c', 'd'];
const down = moveItem(src, 0, 2);
if (down.join('') !== 'bcad') throw new Error('Перемещение 0→2 должно дать b,c,a,d, а дало ' + down.join(','));
const up = moveItem(src, 3, 1);
if (up.join('') !== 'adbc') throw new Error('Перемещение 3→1 должно дать a,d,b,c, а дало ' + up.join(','));
const noop = moveItem(src, 0, 9);
if (noop.join('') !== 'abcd') throw new Error('Индекс вне диапазона → массив без изменений');
if (src.join('') !== 'abcd') throw new Error('Исходный массив нельзя мутировать');
if (noop === src) throw new Error('Нужно возвращать новый массив, а не ту же ссылку');
'Отлично: пункт перемещается без мутаций'`,
  },
  {
    id: 'c9',
    title: 'Статус формы → доступные действия',
    blurb:
      'Логика доступности действий по статусу — типичная работа в формах. Напишите availableActions(status), возвращающую отсортированный массив разрешённых действий. Правила: "draft" → delete, edit, submit; "rejected" → edit, submit; "submitted" и "approved" → только view. Для неизвестного статуса выбросьте Error("unknown status").',
    starter: `function availableActions(status) {
  // вернуть отсортированный массив действий либо бросить Error('unknown status')
}`,
    hiddenTests: `
const eq = (a, b) => a.length === b.length && a.every((x, i) => x === b[i]);
const cases = [
  ['draft', ['delete', 'edit', 'submit']],
  ['rejected', ['edit', 'submit']],
  ['submitted', ['view']],
  ['approved', ['view']],
];
for (const [status, expected] of cases) {
  const got = availableActions(status);
  if (!Array.isArray(got) || !eq(got, expected)) {
    throw new Error(status + ' дал ' + JSON.stringify(got) + ', а нужно ' + JSON.stringify(expected));
  }
}
let threw = false;
try { availableActions('weird'); } catch (e) { threw = e.message === 'unknown status'; }
if (!threw) throw new Error('Для неизвестного статуса ожидался Error("unknown status")');
'Отлично: действия зависят от статуса'`,
  },
  {
    id: 'c10',
    title: 'Валидация: найти дубли кодов в списке',
    blurb:
      'Перед сохранением списка нужно подсветить повторяющиеся коды. Напишите findDuplicateCodes(points): по массиву объектов { code } верните массив кодов, встречающихся более одного раза. Каждый код — один раз, в порядке первого появления. Если дублей нет — пустой массив.',
    starter: `function findDuplicateCodes(points) {
  // points: Array<{ code: string }>
  // вернуть: string[]
}`,
    hiddenTests: `
const dup = findDuplicateCodes([{ code: 'A' }, { code: 'B' }, { code: 'A' }, { code: 'C' }, { code: 'B' }]);
if (dup.join(',') !== 'A,B') throw new Error('Ожидались дубли A,B в порядке появления, а дало ' + dup.join(','));
const none = findDuplicateCodes([{ code: 'X' }, { code: 'Y' }]);
if (none.length !== 0) throw new Error('Без дублей ожидался пустой массив');
const empty = findDuplicateCodes([]);
if (empty.length !== 0) throw new Error('Пустой вход → пустой массив');
const triple = findDuplicateCodes([{ code: 'Z' }, { code: 'Z' }, { code: 'Z' }]);
if (triple.join(',') !== 'Z') throw new Error('Код-дубль должен появиться в результате один раз');
'Отлично: дубли находятся корректно'`,
  },
  {
    id: 'c11',
    title: 'i18n: перевод с подстановкой параметров',
    blurb:
      'Мини-версия того, что делает Transloco. Напишите t(dict, key, params): dict — плоский словарь переводов, значения могут содержать плейсхолдеры вида {{name}}. Верните строку с подставленными значениями из params. Если ключа нет в словаре — верните сам key (поведение по умолчанию). params может отсутствовать.',
    starter: `function t(dict, key, params = {}) {
  // dict: { [key: string]: string }, значения c плейсхолдерами {{param}}
}`,
    hiddenTests: `
const dict = { 'form.title': 'Заголовок', greeting: 'Привет, {{name}}!', sum: '{{a}} + {{b}}' };
if (t(dict, 'form.title') !== 'Заголовок') throw new Error('Простой ключ без параметров');
if (t(dict, 'greeting', { name: 'Дарья' }) !== 'Привет, Дарья!') throw new Error('Подстановка одного параметра');
if (t(dict, 'sum', { a: 2, b: 3 }) !== '2 + 3') throw new Error('Подстановка нескольких параметров (числа приводятся к строке)');
if (t(dict, 'missing.key') !== 'missing.key') throw new Error('Нет ключа → возвращаем сам ключ');
'Отлично: перевод с подстановкой работает'`,
  },
];
